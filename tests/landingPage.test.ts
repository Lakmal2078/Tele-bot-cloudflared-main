...
describe("Landing Page Render & SEO", () => {
  ...
  it("renders deep-linked Telegram bot CTAs", () => {
    ...
    expect(html).toContain("stickyCta");
  });
  ...
  it("renders tips preview with loading skeleton (no static samples)", () => {
    const req = new Request("https://fast-xbet.lk/");
    const html = renderLandingPage(mockEnv, req);
    expect(html).toContain('id="tips-preview"');
    expect(html).toContain('id="tips-root"');
    expect(html).toContain("tipSkeleton");
    expect(html).not.toContain("Arsenal");
    expect(html).not.toContain("Chelsea");
  });
