...
function selectPaidFeeds(explicitSports: string[], discoveredSports: string[], maxFeeds: number): string[] {
  ...
  // Explicit feeds are operator-controlled and therefore have priority.
  for (const sport of explicitSports) add(sport);
  // Auto-discovered feeds fill the remaining paid-request budget.
...
async function fetchCandidates(env: Env): Promise<TipCandidate[]> {
  ...
  const sports = selectPaidFeeds(explicitSports, discovered, MAX_ODDS_FEEDS_PER_SLOT);
  ...
  console.log(`[Tips] Credit guard: paid_requests=${usage.paidRequestAttempts}/${MAX_ODDS_FEEDS_PER_SLOT}, remaining=${usage.remaining ?? "unknown"}, used=${usage.usedFromHeader ?? "unknown"}`);
  return selected;
}

function formatKickoff(iso: string): string { return new Intl.DateTimeFormat("en-GB", { timeZone: "Asia/Colombo", dateStyle: "medium", timeStyle: "short" }).format(new Date(iso)); }
...
function formatTipMessage(candidates: TipCandidate[], slot: string, joinUrl?: string): string { const lines = ["🇱🇰 <b>FREE TIPS — SRI LANKA</b>", `🕐 <b>${escapeHtml(slot)} Sri Lanka time</b>`, `🎯 <b>${candidates.length} quality tip${candidates.length === 1 ? "" : "s"}</b>`, "━━━━━━━━━━━━━━━━━━"]; candidates.forEach((candidate, index) => { lines.push(`${candidate.emoji} <b>TIP ${index + 1} — ${escapeHtml(candidate.event.sport_title)}</b>`, `⚔️ ${escapeHtml(candidate.event.home_team)} vs ${escapeHtml(candidate.event.away_team)}`, `🗓️ ${escapeHtml(formatKickoff(candidate.event.commence_time))} (SL)`, `🎯 <b>Pick:</b> ${escapeHtml(candidate.selection)}`, `📊 <b>Market:</b> ${escapeHtml(candidate.market.toUpperCase())}`, `💹 <b>Average odds:</b> ${candidate.averageOdds.toFixed(2)}`, `🏪 <b>Bookmakers:</b> ${candidate.bookmakerCount}`, `📈 <b>Market probability:</b> ${(candidate.impliedProbability * 100).toFixed(1)}%`, "━━━━━━━━━━━━━━━━━━"); }); lines.push("ℹ️ Picks are ranked using bookmaker-market data. Market probability is not a guarantee or a prediction model.", "⚠️ <b>Bet responsibly.</b> Never stake money you cannot afford to lose."); if (joinUrl) lines.push(`\n<a href="${escapeHtml(joinUrl)}">📣 Join our channel for more free tips</a>`); return lines.join("\n"); }
function formatFallbackTip(slot: string, joinUrl?: string): string { const lines = ["🇱🇰 <b>FREE TIPS UPDATE</b>", `🕐 <b>${escapeHtml(slot)} Sri Lanka time</b>`, "━━━━━━━━━━━━━━━━━━", "ℹ️ No qualifying matches were available for this slot.", "✅ We do not publish forced or low-confidence picks.", "📢 The next scheduled update will be published automatically.", "", "⚠️ Bet responsibly and only what you can afford to lose."]; if (joinUrl) lines.push(`\n<a href="${escapeHtml(joinUrl)}">📣 Join our channel</a>`); return lines.join("\n"); }
async function postTelegramMessage(env: Env, message: string): Promise<number | null> { const response = await fetch(`https://api.telegram.org/bot${encodeURIComponent(env.BOT_TOKEN)}/sendMessage`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ chat_id: env.TIPS_CHANNEL_ID, text: message, parse_mode: "HTML", disable_web_page_preview: true }) }); const telegram = await response.json() as { ok: boolean; description?: string; result?: { message_id?: number } }; if (!response.ok || !telegram.ok) throw new Error(`Telegram send failed: ${telegram.description || response.status}`); return telegram.result?.message_id ?? null; }
